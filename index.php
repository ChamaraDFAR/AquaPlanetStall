...
<html lang="en">
  <head>
    ...
    <style>
      ...
      .legend-box { width: 1.1rem; height:1.1rem; border-radius:.25rem; border:2px solid; }
      /* Road */
      .road { height: .9rem; background:#333; border-radius:.25rem; position:relative; }
      .road:after { content:""; position:absolute; inset:0 .5rem; top:50%; height:0; border-top:4px dashed rgba(255,255,255,.7); transform:translateY(-50%); }
      /* Stalls */
      .stall-box { width: 3.2rem; height: 2.6rem; font-weight: 700; font-size: .8rem; border: 1px solid #d1d9e6; border-radius: .5rem; display:flex; align-items:center; justify-content:center; background:#fff; color:#334155; transition: transform .12s ease, background-color .12s ease, border-color .12s ease; user-select: none; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
      .stall-available { cursor: pointer; background:#fff; }
      .stall-available:hover { background: #f8fafc; border-color: #9bbcf9; }
      .stall-available:focus { outline: none; }
      .stall-available:focus-visible { outline: 2px solid #93c5fd; outline-offset: 2px; }
      .stall-selected-dfar { background: #0ea5e9; color: #fff; border-color: #0284c7; transform: translateY(-1px); box-shadow: 0 2px 6px rgba(2,132,199,.35); }
      .stall-selected-naqda { background: #6366f1; color: #fff; border-color: #4f46e5; transform: translateY(-1px); box-shadow: 0 2px 6px rgba(79,70,229,.35); }
      .stall-booked { background: #eef2f7; color: #6b7280; border-color: #d1d9e6; text-decoration: line-through; cursor:not-allowed; box-shadow: none; }
      .stall-large { width: 3.6rem; height: 2.9rem; font-size: .9rem; border-width: 2px; }
      .grid-cols-5, .grid-cols-7, .grid-cols-14, .grid-cols-15 { gap:.35rem; }
      .grid-cols-15.stretch { gap:.45rem; }
      .badge-panel { border:1px solid #d1d9e6; }
      /* Bushes */
      .bush { width: 2.5rem; height: 1.2rem; background: #22c55e; border-top-left-radius: 9999px; border-top-right-radius: 9999px; }
      .grid-cols-5 { display:grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap:.4rem; }
      .grid-cols-7 { display:grid; grid-template-columns: repeat(7, minmax(0, 1fr)); gap:.4rem; }
      .grid-cols-14 { display:grid; grid-template-columns: repeat(14, minmax(0, 1fr)); gap:.4rem; }
      .grid-cols-15 { display:grid; grid-template-columns: repeat(15, minmax(0, 1fr)); gap:.4rem; }
      .section-label { width: 2rem; text-align:center; font-weight:800; font-size:1.75rem; color:#9aa3b2; }
      .card-soft { border:0; box-shadow: 0 8px 24px rgba(0,0,0,.06); border-radius:.75rem; }
      .badge-panel { border:2px solid #cbd5e1; border-radius:.5rem; padding:.4rem .6rem; background:#f8fafc; font-weight:700; }
      .vert-label { writing-mode: vertical-rl; transform: rotate(180deg); letter-spacing:.08em; color:#64748b; font-weight:700; }
    </style>
  </head>
  <body>
    <div class="container py-4">
      <header class="mb-2 text-center">
        <h1 class="h3 fw-bold text-dark">Exhibition Stall Booking</h1>
        <p class="text-muted">Select your desired stalls from the map below.</p>
      </header>
      ...
      <div class="row g-4">
            ...
            <div class="card-body">
              <div class="d-flex align-items-center gap-4 flex-wrap mb-3">
                ...
                <div class="d-flex align-items-center gap-2">
                  ...
                  <span class="small">Selected (DFAR)</span>
                </div>
                <div class="d-flex align-items-center gap-2">
                  <div class="legend-box" style="border-color:#3c36a6; background:#6366f1;"></div>
                  <span class="small">Selected (NAQDA)</span>
                </div>
                <div class="d-flex align-items-center gap-2">
        ...
        <div class="col-12 col-xl-4">
          <div class="card card-soft position-sticky" style="top:1rem;">
            <div class="card-body">
              <h2 class="h5 fw-bold border-bottom pb-2 mb-3">Your Selection</h2>
              ...
              <div id="selection-empty" class="text-muted text-center py-4">Click on an available stall to add it to your booking.</div>
    ...
    <script>
      ...
      function renderMap(){
        ...
        ;['P','Q','R','S','T'].forEach(section => {
          ...
          const col = document.createElement('div'); col.className = 'd-flex flex-column gap-1';
        ...
        const mTop = document.createElement('div'); mTop.className = 'grid-cols-14'; getByPrefix('V').slice(75,89).forEach(s => mTop.appendChild(createStallButton(s)));
        const mGrid = document.createElement('div'); mGrid.className = 'grid-cols-15 stretch'; const m = getByPrefix('V');
        ...
        const stage = document.createElement('div'); stage.className = 'badge-panel fw-bold'; stage.textContent = 'MAIN STAGE';
